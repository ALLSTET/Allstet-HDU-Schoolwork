// [compile]
// c++ -o separate_obj -O3 -Wall separate_obj.cpp `sdl2-config --cflags --libs`
// [/compile]
...
public:
    ...
    Camera(Vec3f eye, Vec3f target, float fov, float ar)
    {
        origin = eye;
        aspect = ar;
        scale = tan(M_PI * 0.5f * fov / 180.0f);
        forward = (target - eye).normalize();
        Vec3f worldUp(0, 1, 0);
        right = Vec3f(worldUp.y * forward.z - worldUp.z * forward.y,
    ...
    Vec3f getRayDir(float x, float y) const
    {
        return (right * x * aspect * scale + up * y * scale + forward).normalize();
    }
};
...
// 修改后的 render 函数：接收 Camera 对象
uint32_t *render(const Camera &cam, const std::vector<Sphere> &spheres, const std::vector<Triangle> &triangles)
{
    ...
    for (unsigned y = 0; y < h; ++y)
    {
        ...
        { // 应用开头定义的采样模式（泊松模式，4x超采样）
            ...
            for (int s = 0; s < samplesPerPixel; ++s)
            {
                ...
                float xx = (2 * ((x + 0.5f + sp.offsetX * 0.5f) * invW) - 1);
                float yy = (1 - 2 * ((y + 0.5f + sp.offsetY * 0.5f) * invH));

                // 使用摄像机获取射线方向
                Vec3f dir = cam.getRayDir(xx, yy);

                Vec3f col = trace(cam.origin, dir, spheres, triangles, 0);
...
// --- 新增：添加椅子函数（此方法支持纹理） ---
void addChair(std::vector<Triangle> &tris, Vec3f centerPos, float width, float height, float depth, Vec3f color, float refl, float shiny, const Texture *tex = nullptr)
{
    float halfW = width * 0.5f;
    float halfD = depth * 0.5f;
    ...
    addBox(tris, Vec3f(centerPos.x - halfW, centerPos.y, centerPos.z - halfD), Vec3f(centerPos.x + halfW, centerPos.y + thickness, centerPos.z + halfD), color, refl, shiny, tex);
    addBox(tris, Vec3f(centerPos.x - halfW, centerPos.y - thickness, centerPos.z - halfD), Vec3f(centerPos.x - halfW + 0.1, centerPos.y + height * 0.8, centerPos.z + halfD), color, refl, shiny, tex);

    addBox(tris, Vec3f(centerPos.x - halfW, centerPos.y - height, centerPos.z + halfD - legWidth), Vec3f(centerPos.x - halfW + legWidth, centerPos.y, centerPos.z + halfD), color, 0, 0, tex);
    addBox(tris, Vec3f(centerPos.x + halfW - legWidth, centerPos.y - height, centerPos.z + halfD - legWidth), Vec3f(centerPos.x + halfW, centerPos.y, centerPos.z + halfD), color, 0, 0, tex);
    addBox(tris, Vec3f(centerPos.x - halfW, centerPos.y - height, centerPos.z - halfD), Vec3f(centerPos.x - halfW + legWidth, centerPos.y, centerPos.z - halfD + legWidth), color, 0, 0, tex);
    addBox(tris, Vec3f(centerPos.x + halfW - legWidth, centerPos.y - height, centerPos.z - halfD), Vec3f(centerPos.x + halfW, centerPos.y, centerPos.z - halfD + legWidth), color, 0, 0, tex);
}
// --- 新增：添加柜子函数（此方法支持纹理） ---
void addCabinet(std::vector<Triangle> &tris, Vec3f centerPos, float width, float height, float depth, Vec3f color, float refl, float shiny, const Texture *tex = nullptr)
{
    float halfW = width * 0.5f;
    float halfD = depth * 0.5f;
...
// segments: 杯子侧壁分段数
void addCup(std::vector<Triangle> &tris, Vec3f center, float radius, float height, Vec3f color, float refl, float shiny, int segments, const Texture *tex = nullptr)
{
    ...
    for (int i = 0; i < segments; ++i)
    {
        ...
        // 2. 仅应用平移 (直接加上 center，不进行旋转)
        Vec3f b1 = center + localB1;
        Vec3f b2 = center + localB2;
        Vec3f t1 = center + localT1;
        Vec3f t2 = center + localT2;

        // 3. 构建侧壁
        addQuad(tris, b1, b2, t2, t1, color, refl, shiny, tex);

        // 4. 构建杯底
        if (tex)
        {
            // 注意：createTexturedTriangle 函数需要你在外部定义好，这里照常调用
...
// 综合函数：构建带洞的后墙 + 自动安窗户 + 自动种树 + 自动打光 + 纹理支持
void addWindowedBackWall(
    ...
    const Texture *tex = nullptr // 添加纹理参数
)
{
    float wallZ = lbb.z;
    ...
    // --- 1. 拆分墙体 (挖出窗户洞) ---
    // A. 下墙
    addQuad(tris, lbb, rbb, Vec3f(rbb.x, winBottomY, wallZ), Vec3f(lbb.x, winBottomY, wallZ), color, 0.0f, 0.0f, tex);
    // B. 上墙
    addQuad(tris, Vec3f(lbb.x, winTopY, wallZ), Vec3f(rbb.x, winTopY, wallZ), rtb, ltb, color, 0.0f, 0.0f, tex);
    // C. 左墙柱
    addQuad(tris, Vec3f(lbb.x, winBottomY, wallZ), Vec3f(winLeftX, winBottomY, wallZ), Vec3f(winLeftX, winTopY, wallZ), Vec3f(lbb.x, winTopY, wallZ), color, 0.0f, 0.0f, tex);
    // D. 右墙柱
    addQuad(tris, Vec3f(winRightX, winBottomY, wallZ), Vec3f(rbb.x, winBottomY, wallZ), Vec3f(rbb.x, winTopY, wallZ), Vec3f(winRightX, winTopY, wallZ), color, 0.0f, 0.0f, tex);
    ...
    spheres.push_back(Sphere(lightPos, 0.5f, Vec3f(0), 0, 0, Vec3f(100, 100, 100), 10.f));
...
void addLeaningPictureFrame(std::vector<Triangle> &tris, Vec3f bottomPos, float width, float height, float tiltOffset, const Texture *tex = nullptr)
{
    float halfW = width * 0.5f;
    ...
    addQuad(tris, c_bl, c_br, c_tr, c_tl, canvasColor, 0.0f, 0.0f, tex);
}

// [新增] 用于保存 OBJ 文件的函数
void saveObj(const std::string &filename, const std::vector<Triangle> &tris)
{
    if (tris.empty())
        return;
    ...
    for (const auto &t : tris)
    {
        ...
        out << "v " << t.v1.x << " " << t.v1.y << " " << t.v1.z << "\n";
        out << "v " << t.v2.x << " " << t.v2.y << " " << t.v2.z << "\n";
        out << "vt " << t.uv0[0] << " " << t.uv0[1] << "\n";
        out << "vt " << t.uv1[0] << " " << t.uv1[1] << "\n";
        out << "vt " << t.uv2[0] << " " << t.uv2[1] << "\n";
        Vec3f n = t.normal();
        out << "vn " << n.x << " " << n.y << " " << n.z << "\n";
        out << "f " << idx << "/" << idx << "/" << idx << " " << idx + 1 << "/" << idx + 1 << "/" << idx + 1 << " " << idx + 2 << "/" << idx + 2 << "/" << idx + 2 << "\n";
        idx += 3;
    }
    out.close();
}
...
int main(int argc, char *argv[])
{
    if (SDL_Init(SDL_INIT_VIDEO) != 0)
        return 1;
    SDL_Window *window = SDL_CreateWindow("Raytracer - Switch Viewpoint", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 800, 600, SDL_WINDOW_SHOWN);
    ...
    std::vector<Camera> cameras = {
        ...
        Camera(Vec3f(8, 2, -5), Vec3f(-5, -2, -40), 55.0f, 800.0f / 600.0f),  // 左侧面略高
        Camera(Vec3f(0, 0, -35), Vec3f(0, 0, -45), 60.0f, 800.0f / 600.0f),   // 正前方
        Camera(Vec3f(-12, 2, -10), Vec3f(0, -3, -30), 40.0f, 800.0f / 600.0f) // 右侧面略高
    };
    int currentCamIndex = 0;

    // --- 容器定义 ---
    std::vector<Triangle> roomTris, tableTris, chairTris, cupTris, pictureTris, lampTris, triangles;
    std::vector<Sphere> spheres;

    // --- 纹理加载 ---
    Texture walltexture, floortexture, ceilingtexture, tabletexture, glasstexture, picturetexture;
    ...
    bool pictureTexLoaded = loadTextureFromFile("textures/picture_texture.jpg", picturetexture);

    // --- 材质定义 ---
    Vec3f wallColor(0.9f, 0.88f, 0.82f), floorColor(0.45f, 0.35f, 0.25f), ceilingColor(0.95f), tableColor(0.2f, 0.15f, 0.1f);

    // --- 建模逻辑 ---
    float wX = 14.0f, fY = -6.0f, cY = 10.0f, bZ = -45.0f, fZ = 5.0f;
    Vec3f lbf(-wX, fY, fZ), lbb(-wX, fY, bZ), ltb(-wX, cY, bZ), ltf(-wX, cY, fZ);
    Vec3f rbf(wX, fY, fZ), rbb(wX, fY, bZ), rtb(wX, cY, bZ), rtf(wX, cY, fZ);

    if (floorTexLoaded)
        addQuad(roomTris, lbf, lbb, rbb, rbf, floorColor, 0.25f, 30.f, &floortexture);
    else
        addQuad(roomTris, lbf, lbb, rbb, rbf, floorColor, 0.25f, 30.f);

    if (ceilingTexLoaded)
        addQuad(roomTris, ltf, ltb, rtb, rtf, ceilingColor, 0.0f, 0.f, &ceilingtexture);
    else
        addQuad(roomTris, ltf, ltb, rtb, rtf, ceilingColor);

    if (wallTexLoaded)
    {
        addQuad(roomTris, lbf, lbb, ltb, ltf, wallColor, 0.0f, 0.f, &walltexture);
        addQuad(roomTris, rbf, rbb, rtb, rtf, wallColor, 0.0f, 0.f, &walltexture);
        addWindowedBackWall(roomTris, spheres, lbb, rbb, rtb, ltb, wallColor, 0.0f, 6.0f, -6.0f, 6.0f, &walltexture);
    }
    else
    {
        addQuad(roomTris, lbf, lbb, ltb, ltf, wallColor);
        addQuad(roomTris, rbf, rbb, rtb, rtf, wallColor);
        addWindowedBackWall(roomTris, spheres, lbb, rbb, rtb, ltb, wallColor, 0.0f, 6.0f, -6.0f, 6.0f);
    }

    if (tableTexLoaded)
        addTable(tableTris, Vec3f(0, fY, -24), 10.0f, 3.0f, 8.0f, tableColor, 0.1f, 10.f, &tabletexture);
    else
        addTable(tableTris, Vec3f(0, fY, -24), 10.0f, 3.0f, 8.0f, tableColor, 0.1f, 10.f);

    if (tableTexLoaded)
        addChair(chairTris, Vec3f(-7, fY + 1.5f, -23), 2.5f, 3.0f, 2.0f, tableColor, 0.1f, 10.f, &tabletexture);
    else
        addChair(chairTris, Vec3f(-7, fY + 1.5f, -23), 2.5f, 3.0f, 2.0f, tableColor, 0.1f, 10.f);

    addCup(cupTris, Vec3f(-1.5f, fY + 3.0f, -24), 1.0f, 2.5f, Vec3f(0.9f, 0.95f, 1.0f), 0.1f, 100.f, 64,  &glasstexture);

    if (pictureTexLoaded)
        addLeaningPictureFrame(pictureTris, Vec3f(10.0f, fY, -42.0f), 4.0f, 5.0f, 2.5f, &picturetexture);
    else
        addLeaningPictureFrame(pictureTris, Vec3f(10.0f, fY, -42.0f), 4.0f, 5.0f, 2.5f);
    ...
    addLampShade(lampTris, lightPos + Vec3f(0, 0.5f, 0), cY);

    // 导出 OBJ
    saveObj("output_Room.obj", roomTris);
    saveObj("output_Table.obj", tableTris);
    saveObj("output_Chair.obj", chairTris);
    saveObj("output_Cup.obj", cupTris);
    saveObj("output_Picture.obj", pictureTris);
    saveObj("output_LampShade.obj", lampTris);

    // 合并
    triangles.insert(triangles.end(), roomTris.begin(), roomTris.end());
    triangles.insert(triangles.end(), tableTris.begin(), tableTris.end());
    triangles.insert(triangles.end(), chairTris.begin(), chairTris.end());
    triangles.insert(triangles.end(), cupTris.begin(), cupTris.end());
    triangles.insert(triangles.end(), pictureTris.begin(), pictureTris.end());
    triangles.insert(triangles.end(), lampTris.begin(), lampTris.end());

    // --- 主循环: 增加按键刷新逻辑 ---
    bool run = true;
    bool dirty = true; // 标记是否需要重新渲染
    SDL_Event e;
    while (run)
    {
        while (SDL_PollEvent(&e))
        {
            ...
            if (e.type == SDL_KEYDOWN)
            {
                if (e.key.keysym.sym == SDLK_SPACE)
                {
                    ...
                    dirty = true;
                    std::cout << "Switching to Camera view: " << currentCamIndex << std::endl;
                }
            }
        }
        ...
        if (dirty)
        {
            uint32_t *buf = render(cameras[currentCamIndex], spheres, triangles);
